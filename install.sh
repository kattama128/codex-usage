...
echo "=== Codex Usage Monitor — installer ==="

# ── 1. system packages (GTK indicator backend for pystray) ────────────────────
echo "[1/4] Checking system packages …"
MISSING=()
for pkg in python3-gi python3-gi-cairo gir1.2-ayatana-appindicator3-0.1 libayatana-appindicator3-1; do
...
pip3 install --user -r "$REPO_DIR/requirements.txt" --quiet

# ── 3. cron job (monitor runs every $CRON_INTERVAL minutes) ──────────────────
echo "[3/4] Setting up cron job (every ${CRON_INTERVAL}m) …"
CRON_CMD="*/$CRON_INTERVAL * * * * python3 $REPO_DIR/codex_monitor.py >> $HOME/.codex-usage/monitor.log 2>&1"
# Remove old entry, add new one
( crontab -l 2>/dev/null | grep -v "codex_monitor.py" ; echo "$CRON_CMD" ) | crontab -
echo "      Cron job added."
...
echo ""
echo "  The monitor also runs automatically every ${CRON_INTERVAL} minutes via cron."
echo "  Cron log: ~/.codex-usage/monitor.log"
echo ""
